...
export default function AddMenuItem() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedMenuItems = [...menuItems, { ...menuItem, photoUrl: downloadURL }];
      ...
      <div className="h-screen bg-gradient-to-b from-red-300 to-orange-200">
        <div className="container mx-auto p-4 flex flex-col justify-center items-center">
          ...
          <div className="mt-10 w-full max-w-lg">
            <h2 className="text-2xl font-bold text-stone-900">Current Menu Items</h2>
            <ul className="space-y-4 mt-4">
              {menuItems.map((item) => (
                <li key={item.id} className="p-4 border border-stone-300 rounded-md shadow-md">
                  <h3 className="font-bold text-lg">{item.name}</h3>
                  <p>{item.description}</p>
                  <p>Price: ${item.price}</p>
                  <p>Rating: {item.rating}</p>
                  <img src={item.photoUrl} alt={item.name} className="mt-2 w-32 h-32 object-cover" />
                </li>
              ))}
            </ul>
          </div>
